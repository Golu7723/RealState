...
public class AuthIntegrationTest extends BaseTestContainerTest {
        ...
        void shouldVerifyUserRegistrationAsAdmin() throws Exception {
                ...
                mockMvc.perform(get("/api/v1/admin/user-management")
                                ...
                                .andExpect(jsonPath("$[?(@.email == '%s')].firstName"
                                                .formatted(testUserList.get(0).getEMAIL()))
                                                .value(testUserList.get(0).getFIRST_NAME()))
                                .andExpect(jsonPath("$[?(@.email == '%s')].lastName"
                                                .formatted(testUserList.get(0).getEMAIL()))
                                                .value(testUserList.get(0).getLAST_NAME()))
                                .andExpect(jsonPath("$[?(@.email == '%s')].mobileNumber"
                                                .formatted(testUserList.get(0).getEMAIL()))
                                                .value(testUserList.get(0).getMOBILE_NUMBER()));
        }
        ...
        void shouldLoginAndLogoutSuccessfully() throws Exception {
                ...
                AuthenticationResponse authenticationResponse = TestUtil.testLogin(mockMvc, objectMapper,
                                testUserList.get(0).getEMAIL(), testUserList.get(0).getPASSWORD());
                ...
                mockMvc.perform(get("/api/v1/user/").header("Authorization", "Bearer " + accessToken))
                ...
                mockMvc.perform(get("/api/v1/user/").header("Authorization", "Bearer " + accessToken))
                                .andExpect(status().is(403)); // Access should be denied
        }
        ...
        void shouldUserResetPasswordSuccessfully() throws Exception {
                AuthenticationResponse authenticationResponse = TestUtil.testLogin(mockMvc, objectMapper,
                                testUserList.get(0).getEMAIL(), testUserList.get(0).getPASSWORD());
                String accessToken = authenticationResponse.getAccessToken();
                TestUtil.testResetPasswordAndLogout(mockMvc, objectMapper, accessToken,
                                testUserList.get(0).getPASSWORD() + "1");
                assertThrows(AssertionError.class, () -> {
                        TestUtil.testLogin(mockMvc, objectMapper, testUserList.get(0).getEMAIL(),
                                        testUserList.get(0).getPASSWORD());
                });
                authenticationResponse = TestUtil.testLogin(mockMvc, objectMapper, testUserList.get(0).getEMAIL(),
                                testUserList.get(0).getPASSWORD() + "1");
        ...
        void shouldCustomerAccessProtectedAuthEndpoints() throws Exception {
                ...
                mockMvc.perform(get("/api/v1/auth/user")
                                .header("Authorization",
                                                "Bearer " + accessToken.substring(0, accessToken.length() - 5)
                                                                + "abcde"))
                                .andExpect(status().isForbidden()); // Expecting HTTP 403 Forbidden
                ...
                s = mockMvc.perform(get("/api/v1/auth/userByAccessToken")
                                ...
                                .getResponse()
                                .getContentAsString();
                RetrieveAccountResponse retrieveAccountResponseFromAccessToken = objectMapper.readValue(s,
                ...
                s = mockMvc.perform(get("/api/v1/auth/userByRefreshToken")
                                ...
                                .getResponse()
                                .getContentAsString();
                RetrieveAccountResponse retrieveAccountResponseFromRefreshToken = objectMapper.readValue(s,
                                RetrieveAccountResponse.class);

                assertEquals(testUserList.get(0).getMOBILE_NUMBER(), loggedUserResponse.getMobileNumber());
                assertEquals(testUserList.get(0).getMOBILE_NUMBER(),
                                retrieveAccountResponseFromRefreshToken.getMobileNumber());
                assertEquals(testUserList.get(0).getMOBILE_NUMBER(),
                                retrieveAccountResponseFromAccessToken.getMobileNumber());

                assertEquals(testUserList.get(0).getFIRST_NAME(), loggedUserResponse.getFirstName());
                assertEquals(testUserList.get(0).getFIRST_NAME(),
                                retrieveAccountResponseFromRefreshToken.getFirstName());
                assertEquals(testUserList.get(0).getFIRST_NAME(),
                                retrieveAccountResponseFromAccessToken.getFirstName());
                TestUtil.testLogout(mockMvc, accessToken);
        }
}
