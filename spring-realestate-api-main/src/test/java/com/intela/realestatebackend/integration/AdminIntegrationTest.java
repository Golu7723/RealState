...
public class AdminIntegrationTest extends BaseTestContainerTest {
        ...
        public void testAdminTestEndpoint() throws Exception {
                AuthenticationResponse authenticationResponse = TestUtil.testLogin(mockMvc, objectMapper,
                                adminUsers.get(0).getEMAIL(), adminUsers.get(0).getPASSWORD());
        ...
        public void testAdminUpdateCustomerProfile() throws Exception {
                byte[] image1Bytes = TestUtil
                                .readFileToBytes(Paths.get(TestUtil.TEST_IMAGE_PATH, "image1.jpg").toString());
                byte[] image2Bytes = TestUtil
                                .readFileToBytes(Paths.get(TestUtil.TEST_IMAGE_PATH, "image2.jpg").toString());

                // Step 1: Admin logs in
                AuthenticationResponse adminAuthResponse = TestUtil.testLogin(mockMvc, objectMapper,
                                adminUsers.get(0).getEMAIL(), adminUsers.get(0).getPASSWORD());
                String adminAccessToken = adminAuthResponse.getAccessToken();

                // Step 2: Customer logs in and retrieves their profile
                AuthenticationResponse customerAuthResponse = TestUtil.testLogin(mockMvc, objectMapper,
                                customerUsers.get(0).getEMAIL(), customerUsers.get(0).getPASSWORD());
                ...
                mockMvc.perform(get("/api/v1/admin/user-management/profiles")
                                .header("Authorization", "Bearer " + adminAccessToken))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$[?(@.userId == " + updatedProfile.getUserId()
                                                + ")].personalDetails.firstName").value("UpdatedFirstName"))
                                .andExpect(jsonPath("$[?(@.userId == " + updatedProfile.getUserId()
                                                + ")].personalDetails.lastName").value("UpdatedLastName"))
                                .andExpect(jsonPath("$[?(@.userId == " + updatedProfile.getUserId()
                                                + ")].contactDetails.contactNumber").value("1234567890"));
        ...
        public void testAdminUpdateCustomerAccount() throws Exception {
                ...
                mockMvc.perform(get("/api/v1/admin/user-management")
                                .header("Authorization", "Bearer " + adminAccessToken))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$[?(@.id == " + updatedAccount.getId() + ")].firstName")
                                                .value("UpdatedFirstName"))
                                .andExpect(jsonPath("$[?(@.id == " + updatedAccount.getId() + ")].lastName")
                                                .value("UpdatedLastName"))
                                .andExpect(jsonPath("$[?(@.id == " + updatedAccount.getId() + ")].mobileNumber")
                                                .value("1234567890"))
                                .andExpect(jsonPath("$[?(@.id == " + updatedAccount.getId() + ")].email")
                                                .value("updatedEmail@gmail.com"));
        ...
        public void testDeleteAccount_Success() throws Exception {
                ...
                String adminAccessToken = adminAuthResponse.getAccessToken();
                AuthenticationResponse customerAuthResponse = TestUtil.testLogin(mockMvc, objectMapper,
                                customerUsers.get(1).getEMAIL(), customerUsers.get(1).getPASSWORD());
                String customerAccessToken = customerAuthResponse.getAccessToken();
                // Step 2: Retrieve a customer account to delete (assume customerUsers.get(0)
                // exists)
                String s = mockMvc.perform(
                                get("/api/v1/user/")
                                                .header("Authorization", "Bearer " + customerAccessToken))
        ...
        public void testDeleteAccount_NonExistentUser() throws Exception {
                ...
                mockMvc.perform(delete("/api/v1/admin/user-management/{userId}", nonExistentUserId)
                                .header("Authorization", "Bearer " + adminAccessToken))
                                .andExpect(status().is4xxClientError()); // Should return 404 since the user does not
                                                                         // exist
                TestUtil.testLogout(mockMvc, adminAccessToken);
        }
        ...
        public void testBanUnbanUser_Success() throws Exception {
                ...
                assertThrows(AssertionError.class, () -> {
                        TestUtil.testLogin(mockMvc, objectMapper, customerUsers.get(0).getEMAIL(),
                                        customerUsers.get(0).getPASSWORD());
                });
                ...
                mockMvc.perform(post("/api/v1/admin/user-management/unban/{userId}", customerUserId)
                                .header("Authorization", "Bearer " + adminAccessToken))
                                .andExpect(status().isOk());
                customerAuthResponse = TestUtil.testLogin(mockMvc, objectMapper, customerUsers.get(0).getEMAIL(),
                                customerUsers.get(0).getPASSWORD());
