...
public class DealerController {
        ...
        @Operation(summary = "Upload a new property with images", description = "Uploads a property JSON and associated image files", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(encoding = {
                        @Encoding(name = "request", contentType = "application/json"),
                        @Encoding(name = "images", contentType = "image/png, image/jpeg")
        }, mediaType = "multipart/form-data", schemaProperties = {
                        @SchemaProperty(name = "request", schema = @Schema(implementation = PropertyRequest.class)),
        ...
        @Operation(summary = "Updates property", description = "Uploads a property JSON object", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(encoding = {
                        @Encoding(name = "request", contentType = "application/json"),
                        @Encoding(name = "images", contentType = "image/png, image/jpeg")
        }, mediaType = "multipart/form-data", schemaProperties = {
                        @SchemaProperty(name = "request", schema = @Schema(implementation = UpdateProfileRequest.class)),
                        @SchemaProperty(name = "images", array = @ArraySchema(schema = @Schema(type = "string", format = "binary")))
        }
        ...
        public ResponseEntity<String> addImageToProperty(@RequestBody MultipartFile[] images,
                        @PathVariable Integer propertyId) {
        ...
        @Operation(summary = "Uploads a new plan with images to property", description = "Uploads a plan JSON and associated image files", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(encoding = {
                        @Encoding(name = "request", contentType = "application/json"),
                        @Encoding(name = "images", contentType = "image/png, image/jpeg")
        }, mediaType = "multipart/form-data", schemaProperties = {
                        @SchemaProperty(name = "request", schema = @Schema(implementation = PlanRequest.class)),
                        @SchemaProperty(name = "images", array = @ArraySchema(schema = @Schema(type = "string", format = "binary")))
        }
        ...
        public ResponseEntity<List<ApplicationResponse>> listAllApplicationsByPropertyId(
                        @PathVariable Integer propertyId) {
                return ResponseEntity.created(URI.create("")).body(
