...
public class AuthService {
    ...
    public AuthenticationResponse register(RegisterRequest request) {
        ...
        validateSupportedAuthRole(normalizedRole);
        ...
        String resolvedFullName = safeValue(request.getFullName());
        String firstName = safeValue(request.getFirstName());
        String lastName = safeValue(request.getLastName());
        if (!resolvedFullName.isBlank()) {
            String[] names = resolvedFullName.split("\\s+");
            firstName = names[0];
            lastName = names.length > 1
                    ? resolvedFullName.substring(firstName.length()).trim()
                    : "User";
        } else {
        ...
        String phone = safeValue(request.getPhone());
        String mobileNumber = !phone.isBlank() ? phone : safeValue(request.getMobileNumber());
    ...
    private Role normalizeRole(Role role) {
        ...
        if (role == Role.DEALER || role == Role.PROPERTY_OWNER) {
            return Role.AGENT;
        }
        return role;
    }
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        validateSupportedAuthRole(requestedRole);
        Role userRole = normalizeRole(user.getRole());
    ...
    private void validateSupportedAuthRole(Role role) {
        if (role != Role.USER && role != Role.AGENT && role != Role.ADMIN) {
            throw new RuntimeException("Invalid role. Allowed roles are USER, AGENT, ADMIN");
        }
    }
