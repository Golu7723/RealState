...
Tomcat `10.1.24` is included in this project at `apache-tomcat-10.1.24` and configured for this codebase.

1. Configure user paths once:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\set-tomcat-paths.ps1
   ```
2. Build and deploy backend WAR to Tomcat:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\deploy-to-tomcat.ps1
   ```
3. Start Tomcat:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\start-tomcat.ps1
   ```
4. Stop Tomcat:
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\stop-tomcat.ps1
   ```

Notes:
- Ensure Java 21 is installed and `JAVA_HOME` is set.
- The app deploys as `ROOT.war`, so it runs at `http://localhost:8080/`.
- Project specific JVM options are set in `apache-tomcat-10.1.24/bin/setenv.bat`.
